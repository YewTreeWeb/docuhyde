...
export const env = (done) => {
	console.log(prod ? 'Running Gulp & Jekyll in production' : 'Running Gulp & Jekyll in development');
	done();
};
...
export const sprite = () => {
	...
	const spriteData = src(config.image.sprites, { allowEmpty: true, since: lastRun(sprite) }).pipe(
		$.spritesmith({
			imgName: 'sprite.png',
			cssName: 'sprite.css'
		})
...
export const svgSprites = () => {
	return src(config.image.svgsprites, { allowEmpty: true, since: lastRun(svgSprites) })
		.pipe($.plumber())
		.pipe($.svgmin())
		.pipe(
			$.rename({
...
export const cloudinary = () => {
	return src(config.cloudinary.src, { allowEmpty: true, since: lastRun(cloudinary) })
		.pipe($.plumber())
		.pipe(
			$.cloudinary({
				config: {
...
export const jekyll = (done) => {
	...
	const build = !prod
		? 'jekyll build --config config/_config.yml, config/_config.dev.yml'
